...
<div id="accountModal" class="modal" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 style="color: #666" class="modal-title">Account Closed!</h5>
                </button>
            </div>
            <div class="modal-body">
                <h3 style="color: #666">Sorry, you reached your stop level.</h3>
            </div>
        </div>
    </div>
</div>
...
function showCurrencyList(searchWord) {
    $.ajax({
        ...
        success: function (data) {
            ...
            if(!forexData.eur_usd.tradeable) {
                $('#simpleModal').modal({ backdrop: "static", keyboard: false }, 'show');
            }
                        ...
                        <div class='d-flex align-items-center justify-content-between sidebar-listening-row' id='stockList_${key}'>
                            <div class='d-flex align-items-center'>
                                ...
                                <h6 class='pl-3 m-0' style='cursor: pointer;' onclick='showStockChart("${key}", "${stockpair}", "stock", "${element.type_stock}")'>
                                    <span>${element.alias || element.base_stock + '/' + element.quote_stock}</span>
                                    <span class='d-none' id='tradingPairShowStock_${key}'>${element.base_stock}/${element.quote_stock}</span>
                                </h6>
                    ...
                    str_crypto = `
                        <div class='d-flex align-items-center justify-content-between sidebar-listening-row' id='cryptoList_${key}'>
                            <div class='d-flex align-items-center'>
                                ...
                                <h6 class='pl-3 m-0' style='cursor: pointer;' onclick='showCryptoChart("${key}", "${cryptopair}", "crypto", "${element.type}", "${element.tradingview_network}")'>
...
function sellAction(trade_kind, modalType) {
    ...
    let trade_price = $("#bid_price_"+modalType+" strong").text() * 1;
    ...
    if(!($("#bid_profit_amount_"+modalType).attr('max') >= $("#bid_profit_amount_"+modalType).val())){
        alert("Invalid Profit Amount!");
        return null;
    }
    if(!($("#bid_loss_amount_"+modalType).attr('min') <= $("#bid_loss_amount_"+modalType).val())){
        alert("Invalid Loss Amount!");
        return null;
    }
...
function buyAction(trade_kind, modalType) {
    ...
    let trade_price = $("#ask_price_"+modalType+" strong").text() * 1;
    let free_margin = $("#free_margin p").text() * 1;
    ...
    if(!($("#ask_loss_amount_"+modalType).attr('max') >= $("#ask_loss_amount_"+modalType).val())){
        alert("Invalid Loss Amount!");
        return null;
    }
    if(!($("#ask_profit_amount_"+modalType).attr('min') <= $("#ask_profit_amount_"+modalType).val())){
        alert("Invalid Profit Amount!");
        return null;
    }
        ...
        success: function (data) {
            ...
            if (res_data != "no") {
                for (let i = 0; i < res_data.length; i++) {
                    ...
                    let exc_rate_custo = 1;
                    //let exchange_rate_cus = getExchangeRateSync(instrumentsAtrCus, res_data[i].kind);
                    if (res_data[i].kind == 'forex') {
                        ...
                        instruments = `<span onclick='showStockChart("${res_data[i].base_symbol + res_data[i].quote_symbol}", "${res_data[i].base_symbol + res_data[i].quote_symbol}", "stock", "STOCK")'>${res_data[i].alias || (res_data[i].base_symbol + "/" + res_data[i].quote_symbol ).toUpperCase()}</span>`;
                    ...
                    if (res_data[i].status == "open") {
                        ...
                        close = `<button class="btn btn-xs btn-outline-danger" id="trade_close_${res_data[i].id}" onclick="closeOrder(${res_data[i].id}, ${market_rate})">Close</button>`;
                        ...
                        function getDataSocket() {
                            ...
                            if (kind == 'forex' || kind == 'stock' || kind == 'crypto') {
                                let data = { data: { bids: 0.0, asks: 0.0 } };
                                if (kind == 'forex'){
                                ...
                                //     }
                                    if (kind == 'forex'){
                                        pro_loss_val = ((units * (market_rate - open_rate)) + spread) / market_rate;
                                    } else {
                                        pro_loss_val = ((units * (market_rate - open_rate)) + spread) / market_rate;
                                    }
                                ...
                                if (res_data[i].profit_switch && res_data[i].profit > 0) {
                                    ...
                                    if (res_data[i].profit <= market_rate) {
                                        closeOrder(res_data[i].id, parseFloat(market_rate.toFixed(2)));
                                    }
                                }
                                ...
                                if (equity == 0) {
                                    ...
                                    if ( (margin > 0) && (equity * 1) < (stop_level * 1) ) {
                                        $('#accountModal').modal({ backdrop: "static", keyboard: false }, 'show');
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    ...
                                    if (res_data[i].profit_switch && res_data[i].profit > 0) {
                                        // if (res_data[i].profit <= pro_loss_val) {
                                        if (res_data[i].profit <= market_rate) {
                                            closeOrder(res_data[i].id, market_rate);
                                        }
                                    }
                                    if (res_data[i].loss_switch && res_data[i].loss > 0) {
                                        // if ( -1 * res_data[i].loss >= pro_loss_val ) {
                    ...
                    if (res_data[i].status == "open") {
                        ...
                        str_opened += `
                        <tr>
                            <td>${res_data[i].ticket}</td>
                            <td>${res_data[i].open_time}</td>
                            <td>${res_data[i].type.toUpperCase()}</td>
                            <td>${units}</td>
                            <td>${instruments}</td>
                            <td>${open_rate}</td>
                            <td>${market_rate_opened_str}</td>
...
function getExchangeRateSync(pair, kind) {
    let rate = 1;
    $.ajax({
        type: "get",
        enctype: "multipart/form-data",
...
function showSellModalForex(id, type) {
    ...
    $("#spreadValue_sell_forex").html(
        "<p>Spread <br/> <span class='theme-color'>" + ((forexData[id].bids - forexData[id].asks) * spread.forex[forexData[id].category] / forexData[id].asks).toFixed(2) + "</span></p>"
    );
    ...
    $('#bid_profit_amount_forex').val($("#forex_sell_" + id).text()).attr({
       "max" : $("#forex_sell_" + id).text(),
       "min" : 0
    });
    $('#bid_loss_amount_forex').val($("#forex_sell_" + id).text()).attr({
       "min" : $("#forex_sell_" + id).text()
    });
}
...
function showSellModalOther(id, type) {
    ...
    $("#spreadValue_sell_other").html(
        "<p>Spread <br/> <span class='theme-color'>" + ((otherData[id].bids - otherData[id].asks) * spread.forex[otherData[id].category]).toFixed(2) + "</span></p>"
    );
    ...
    $('#bid_profit_amount_other').val($("#other_sell_" + id).text()).attr({
       "max" : $("#other_sell_" + id).text(),
       "min" : 0
    });
    $('#bid_loss_amount_other').val($("#other_sell_" + id).text()).attr({
       "min" : $("#other_sell_" + id).text()
    });
}
...
function showSellModalStock(id, type) {
    ...
    $("#spreadValue_sell_stock").html(
        "<p>Spread <br/> <span class='theme-color'>" + ((stockData[id].bids - stockData[id].asks) * spread.stock[stockData[id].category]).toFixed(2) + "</span></p>"
    );
    ...
    $('#bid_profit_amount_stock').val($("#stock_sell_" + id).text()).attr({
       "max" : $("#stock_sell_" + id).text(),
       "min" : 0
    });
    $('#bid_loss_amount_stock').val($("#stock_sell_" + id).text()).attr({
       "min" : $("#stock_sell_" + id).text()
    });
}
...
function showSellModalCrypto(id) {
    ...
    $('#bid_profit_amount_crypto').val($("#crypto_sell_" + id).text()).attr({
       "max" : $("#crypto_sell_" + id).text(),
       "min" : 0
    });
    $('#bid_loss_amount_crypto').val($("#crypto_sell_" + id).text()).attr({
       "min" : $("#crypto_sell_" + id).text()
    });
}
...
function showBuyModalForex(id, type) {
    ...
    $("#spreadValue_buy_forex").html(
        "<p>Spread <br/> <span class='theme-color'>" + ((forexData[id].bids - forexData[id].asks) * spread.forex[forexData[id].category] / forexData[id].bids).toFixed(2) + "</span></p>"
    );
    ...
    $("#bid_amount_forex").val(0);
    $('#ask_profit_amount_forex').val($("#forex_buy_" + id).text()).attr({
       "min" : $("#forex_buy_" + id).text()
    });
    $('#ask_loss_amount_forex').val($("#forex_buy_" + id).text()).attr({
       "max" : $("#forex_buy_" + id).text(),
       "min" : 0
    });
}
...
function showBuyModalOther(id, type) {
    ...
    $("#bid_amount_other").val(0);
    $('#ask_profit_amount_other').val($("#other_buy_" + id).text()).attr({
       "min" : $("#other_buy_" + id).text()
    });
    $('#ask_loss_amount_other').val($("#other_buy_" + id).text()).attr({
       "max" : $("#other_buy_" + id).text(),
       "min" : 0
    });
}
...
function showBuyModalStock(id, type) {
    ...
    $("#bid_amount_stock").val(0);
    $('#ask_profit_amount_stock').val($("#stock_buy_" + id).text()).attr({
       "min" : $("#stock_buy_" + id).text()
    });
    $('#ask_loss_amount_stock').val($("#stock_buy_" + id).text()).attr({
       "max" : $("#stock_buy_" + id).text(),
       "min" : 0
    });
}
...
function showBuyModalCrypto(id) {
    ...
    $("#bid_amount_crypto").val(0);
    $('#ask_profit_amount_crypto').val($("#crypto_buy_" + id).text()).attr({
       "min" : $("#crypto_buy_" + id).text()
    });
    $('#ask_loss_amount_crypto').val($("#crypto_buy_" + id).text()).attr({
       "max" : $("#crypto_buy_" + id).text(),
       "min" : 0
    });
}
...
function showCryptoChart(id, cryptoPair, chart_type, cryptoType, tvNetwork) {
    trade_currency_id = id;
    trade_currency_kind = chart_type;
    ...
    if (chart_type == "crypto") {
        symbol = tvNetwork.toUpperCase() + ":" + cryptoPair;
    }
...
function showSellModal_() {
    if (trade_currency_id != '') {
        switch (trade_currency_kind) {
            case "forex":
                console.log(forexData[trade_currency_id])
...
function closeOrder(id, market_rate) {
    $.ajax({
        type: "get",
        enctype: "multipart/form-data",
        url: base_url + "/trading/close_order",
        data: { id: id, market_rate: market_rate },
...
jQuery(function () {
    ...
    $("#bid_amount_forex").on('keyup change', function () {
        ...
        let required_margin_forex = ((bid_open_rate_forex * bid_amount_forex) / leverage_sell_forex) / bid_open_rate_forex;
        required_margin_forex = parseFloat(required_margin_forex.toFixed(5));

        let pair_id = $("#bid_pair_forex").text().replace(/\//, "_").toLowerCase();
        $("#spreadValue_sell_forex").html(
            "<p>Spread <br/> <span class='theme-color'>" + (((forexData[pair_id].bids - forexData[pair_id].asks) * spread.forex[forexData[pair_id].category] ) * bid_amount_forex).toFixed(2) + "</span></p>"
        );
    ...
    $("#bid_amount_other").on('keyup change', function () {
        ...
        $("#spreadValue_sell_other").html(
            "<p>Spread <br/> <span class='theme-color'>" + (((otherData[pair_id].bids - otherData[pair_id].asks) * spread.forex[otherData[pair_id].category] ) * bid_amount).toFixed(2) + "</span></p>"
        );
    ...
    $("#bid_amount_stock").on('keyup change', function () {
        ...
        $("#spreadValue_sell_stock").html(
            "<p>Spread <br/> <span class='theme-color'>" + (((stockData[pair_id].bids - stockData[pair_id].asks) * spread.stock[stockData[pair_id].category] ) * bid_amount).toFixed(2) + "</span></p>"
        );
    ...
    $("#ask_amount_forex").on('keyup change', function () {
        ...
        let required_margin_forex = ((ask_open_rate_forex * ask_amount_forex) / leverage_buy_forex) / ask_open_rate_forex;
        required_margin_forex = parseFloat(required_margin_forex.toFixed(2));

        let pair_id = $("#ask_pair_forex").text().replace(/\//, "_").toLowerCase();
        $("#spreadValue_buy_forex").html(
            "<p>Spread <br/> <span class='theme-color'>" + (((forexData[pair_id].bids - forexData[pair_id].asks) * spread.forex[forexData[pair_id].category] ) * ask_amount_forex).toFixed(2) + "</span></p>"
        );
    ...
    $("#ask_amount_other").on('keyup change', function () {
        ...
        $("#spreadValue_buy_other").html(
            "<p>Spread <br/> <span class='theme-color'>" + (((otherData[pair_id].bids - otherData[pair_id].asks) * spread.forex[otherData[pair_id].category] ) * ask_amount).toFixed(2) + "</span></p>"
        );
    ...
    $("#ask_amount_stock").on('keyup change', function () {
        ...
        $("#spreadValue_buy_stock").html(
            "<p>Spread <br/> <span class='theme-color'>" + (((stockData[pair_id].bids - stockData[pair_id].asks) * spread.stock[stockData[pair_id].category] ) * ask_amount).toFixed(2) + "</span></p>"
        );
    ...
    $(".bid_minus").on('click', function () {
        ...
        if (modalType == 'forex') {
            let pair_id = $("#bid_pair_forex").text().replace(/\//, "_").toLowerCase();
            $("#spreadValue_sell_forex").html(
                "<p>Spread <br/> <span class='theme-color'>" + (((forexData[pair_id].bids - forexData[pair_id].asks) * spread.forex[forexData[pair_id].category] ) * bid_amount).toFixed(2) + "</span></p>"
            );
        ...
        required_margin = parseFloat(required_margin.toFixed(2));
    ...
    $(".ask_minus").on('click', function () {
        ...
        if (modalType == 'forex') {
            let pair_id = $("#ask_pair_forex").text().replace(/\//, "_").toLowerCase();
            $("#spreadValue_buy_forex").html(
                "<p>Spread <br/> <span class='theme-color'>" + ((((forexData[pair_id].bids - forexData[pair_id].asks) * spread.forex[forexData[pair_id].category] ) * ask_amount_forex).toFixed(2))/ forexData[pair_id].asks + "</span></p>"
            );
        } else if (modalType == 'other') {
            let pair_id = $("#ask_pair_other").text().replace(/\//, "_").toLowerCase();
            $("#spreadValue_buy_other").html(
...
function runWebSocketCrypto(cryptoData) {
    let smplCryptoData = [];
    ...
    let dataPairs = smplCryptoData.map((item) => "XQ." + item.element.base + "-" + item.element.quote);

    const ws = new WebSocket("wss://socket.polygon.io/crypto");

    ws.onopen = () => {
        const auth = {action:"auth", params:"A6CSmtpKreSUjaRRCzcvOYMyjBYM1yJp"};
        ws.send(JSON.stringify(auth));
        const msg = { action: 'subscribe', params: dataPairs.join(',') };
        ws.send(JSON.stringify(msg));
    };
