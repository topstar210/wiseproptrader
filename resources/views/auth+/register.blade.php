...
<body class="crypt-dark" style="background: url('/landingAssets/images/signup_bg.jpg'); background-repeat: no-repeat; background-size:cover">
		...
		<div class="row">
			...
			<div class="col-md-6">
				<div class="cryptorio-forms cryptorio-forms-dark text-center pt-5 pb-5">
					...
					<h3 class="p-4">TradePlus</h3>
					<div class="cryptorio-main-form">
						<form method="POST" action="{{ route('register') }}" class="text-left" id="login_form">
							@csrf
							...
							<select class="custom-select select-background-icon" id="country" name="country" style="background-color: rgb(14,22,45); height:48px; margin-bottom: 10px">
								<option>{{ __('select_your_country') }}</option>
							...
							{{ html()->label(__('ref_code') . '<span class="text-info"> (Optional)</span>')->class('form-control-label pt-3')->for('referalCode') }}
							{{ html()->text('referalCode')
									->class('form-control')
									->placeholder(__('ref_code'))
									->attribute('maxlength', 15) }}

							<input type="submit" value="{{ __('Register') }}" class="crypt-button-red-full" style="margin-top: 40px;">
						</form>
						<p class="float-left"><a href="{{ route('login') }}">{{ __('login') }}</a></p>
